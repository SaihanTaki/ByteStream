...
server.listen(5)
print(f"[*] Listening as {HOST}:{PORT}")  # Infinite loop


connection, address = server.accept()  # Connection = Client
...
if status == 'recv':
    ...
    send_info(connection, filename, filesize)
    progress = progress_info(filename, filesize, status)
    send_file(connection, filename, progress)
    connection.close()
    server.close()
